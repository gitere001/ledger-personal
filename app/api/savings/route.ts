...
export async function GET(req: Request) {
  ...
  if (!all && !month) return NextResponse.json({ error: 'month or all=true required' }, { status: 400 })
  ...
  const allRows: Row[] = all ? rows : await prisma.personalSaving.findMany({})
  ...
  return NextResponse.json({
    rows,
    stats: { totalDeposited, totalInterest, totalWithdrawn, balance },
    accountTotals,
  })
}
